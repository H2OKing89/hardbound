...
from .utils.logging import get_logger, bind_audiobook_context
...
def build_filename(tokens: Tokens, include_subtitle: bool = True, 
                  include_year: bool = True, include_author: bool = True, 
    ...
    filename = f"{left} {' '.join(right)}{tokens.ext}" if right else f"{left}{tokens.ext}"
...
def build_folder_name(tokens: Tokens, include_subtitle: bool = True,
                     include_year: bool = True, include_author: bool = True) -> str:
...
def build_dst_paths(src: Path, dst_root: Path, extension: Optional[str] = None) -> tuple[Path, Path]:
    """
    ...
    logger.debug("trim.start", subtitle=tokens.subtitle, year=tokens.year, author=tokens.author, tag=tokens.tag)
    
    # Stage A: Try different filename trimming levels
    filename_configs = [
        # Full filename (all components)
        {'include_subtitle': True, 'include_year': True, 'include_author': True, 'include_tag': True},
        # Remove year from filename
        {'include_subtitle': True, 'include_year': False, 'include_author': True, 'include_tag': True},
        # Remove year and author from filename  
        {'include_subtitle': True, 'include_year': False, 'include_author': False, 'include_tag': True},
        # Remove year, author, and tag from filename
        {'include_subtitle': True, 'include_year': False, 'include_author': False, 'include_tag': False},
        # Remove everything optional from filename (minimal)
        {'include_subtitle': False, 'include_year': False, 'include_author': False, 'include_tag': False},
    ]
    ...
    minimal_filename = build_filename(tokens, 
                                    include_subtitle=False, include_year=False, 
                                    include_author=False, include_tag=False)
    ...
    for j, folder_config in enumerate(folder_configs):
        ...
        if _fits_red_cap(folder_name, minimal_filename):
            folder_trim_steps = ["drop year", "drop author", "drop tag", "drop subtitle(file)"]  # Already applied minimal filename
...
def validate_path_length(dst_dir: Path, dst_file: str, path_cap: int = PATH_CAP) -> bool:
    """
