...
class TestArgumentParsing:
    ...
    def test_help_output(self):
        ...
        result = subprocess.run([
            sys.executable, str(Path(__file__).parent.parent / "hardbound.py"), "--help"
    ...
    def test_version_output(self):
        ...
        result = subprocess.run([
            sys.executable, str(Path(__file__).parent.parent / "hardbound.py"), "--no-color"
        ], capture_output=True, text=True, input="q\n")  # Send 'q' to quit interactive mode

        # Should not crash
        assert result.returncode == 0 or result.returncode == 1  # 1 is ok for interrupted interactive mode

    def test_invalid_args_error(self):
        """Test that invalid arguments produce errors"""
        result = subprocess.run([
            sys.executable, str(Path(__file__).parent.parent / "hardbound.py"),
            "--src", "/nonexistent", "--dst", "/dst", "--commit", "--dry-run"
        ], capture_output=True, text=True)
